...
def da_rnn(train_data: TrainData, n_targs: int, encoder_hidden_size=64, decoder_hidden_size=64,
           T=20, learning_rate=0.01, batch_size=64):
    ...
    #     lr=learning_rate)
    da_rnn_net = DaRnnNet(encoder, encoder_optimizer)
...
def adjust_learning_rate(net: DaRnnNet, n_iter: int):
    ...
    if n_iter % 10000 == 0 and n_iter > 0:
        for enc_params in net.enc_opt.param_groups:
...
def train_iteration(t_net: DaRnnNet, loss_func: typing.Callable, X, y_history, y_target):
    ...
    y_pred = t_net.encoder(numpy_to_tvar(X))
...
def predict(t_net: DaRnnNet, t_dat: TrainData, train_size: int, batch_size: int, T: int, on_train=False):
    ...
    for y_i in range(0, len(y_pred), batch_size):
        ...
        y_pred[y_slc] =  t_net.encoder(numpy_to_tvar(X)).cpu().data.numpy()
        # _, input_encoded = t_net.encoder(numpy_to_tvar(X))
...
iter_loss, epoch_loss = train(model, data, config, n_epochs=100, save_plots=save_plots)
